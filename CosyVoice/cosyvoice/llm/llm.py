...
class Qwen2Encoder(torch.nn.Module):
    def __init__(self, pretrain_path):
        super().__init__()
        # Force a stable attention backend by default to avoid SDPA sliding-window runtime warnings.
        config = AutoConfig.from_pretrained(pretrain_path)
        if not bool(getattr(config, "use_sliding_window", False)):
            config.sliding_window = None
        attn_impl = str(os.getenv("COSYVOICE_QWEN_ATTN_IMPL", "eager")).strip() or "eager"
